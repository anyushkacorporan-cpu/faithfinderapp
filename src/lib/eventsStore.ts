...
/**
 * The two currencies the app is released in. Stored per event rather than
 * inferred at display time: an event's price is set once, in the organiser's
 * own money, and must keep meaning that no matter who is looking at it.
 */
export type Currency = 'USD' | 'CAD';

/** What the symbol alone cannot say, since both countries write it '$'. */
...
export type AppEvent = {
  ...
  ticketsSold: number;
  /**
   * Seats available, or 0 for uncapped.
   *
   * Optional because every event stored before this existed has no value for
   * it, and those are uncapped by definition rather than sold out.
   */
  capacity?: number;
  currency?: Currency;
  bannerImage?: string;
 ...
 */
export function recordTicketSale(eventId: string, quantity: number): boolean {
  if (!eventId || quantity <= 0) return false;

  const event = events.find(e => e.id === eventId);
  if (!event) return false;

  // Guard here rather than only in the checkout screen. Two people can reach
  // the last seat at once, and a screen that checked before navigating has
  // already stopped looking by the time the sale lands.
  const left = seatsLeft(event);
  if (left !== null && quantity > left) return false;
 ...
 *
 * Null and zero mean opposite things here — null is "no limit set", zero is
 * "sold out" — so callers must not treat a falsy value as either.
 */
export function seatsLeft(event: Pick<AppEvent, 'capacity' | 'ticketsSold'>): number | null {
  const cap = event.capacity ?? 0;
  if (cap <= 0) return null;
  return Math.max(0, cap - (event.ticketsSold || 0));
}

export function isSoldOut(event: Pick<AppEvent, 'capacity' | 'ticketsSold'>): boolean {
