...
# Install date picker
npx expo install @react-native-community/datetimepicker 2>/dev/null || true
echo "packages installed"

cat > app/create-event.tsx << 'ENDOFFILE'
        ...
        <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          ...
          {step===0&&(
            <>
              <TouchableOpacity style={s.bannerPicker} onPress={handlePickBanner}>
                {bannerImage
                  ...
                  :<LinearGradient colors={gradient} style={s.bannerGradient} start={{x:0,y:0}} end={{x:1,y:1}}>
                    ...
                    <Text style={s.bannerTxt}>Add Event Banner</Text>
              ...
              {/* Recurrence */}
              <Text style={s.fieldLbl}>Repeat</Text>
          ...
          {step===1&&(
            <>
              ...
              <Text style={s.fieldLbl}>Speakers & Guests</Text>
              {speakers.map((sp,i)=>(
          ...
          {step===2&&(
            <>
              <Text style={s.fieldLbl}>Venue Type</Text>
              ...
              <TouchableOpacity style={s.uploadBox} onPress={handlePickVenueLayout}>
                {venueLayoutImage
                  ...
                  :<View style={s.uploadBoxInner}>
                    ...
                    <Text style={s.uploadBoxTxt}>Upload seating chart or venue map</Text>
                  </View>}
              </TouchableOpacity>
            </>
          )}
          ...
          {step===3&&(
            <>
              <Text style={s.agendaDesc}>Build your event timeline step by step.</Text>
              ...
              {agendaItems.length===0&&(
                <View style={s.emptyAgenda}>
                  ...
                  <Text style={s.emptyAgendaTxt}>No agenda yet</Text>
            ...
            <>
              <View style={s.ticketRow}>
                <TouchableOpacity style={[s.ticketBtn,!isPaid&&s.ticketBtnFree]} onPress={()=>setIsPaid(false)}>
                  ...
                  <Text style={s.ticketBtnSub}>No payment</Text>
                </TouchableOpacity>
                <TouchableOpacity style={[s.ticketBtn,isPaid&&s.ticketBtnPaid]} onPress={()=>setIsPaid(true)}>
                  ...
                  <Text style={[s.ticketBtnSub,isPaid&&{color:'rgba(255,255,255,0.6)'}]}>Set price</Text>
                </TouchableOpacity>
              </View>
              ...
              {/* Event preview card */}
              <Text style={s.fieldLbl}>Event Preview</Text>
              <Text style={s.previewHint}>This is how your event will appear in the Events tab</Text>
              <EventPreviewCard
      ...
      <Modal visible={showPreview} animationType="slide" presentationStyle="pageSheet">
        <SafeAreaView style={{flex:1,backgroundColor:'#f8f7f4'}} edges={['top']}>
          <View style={s.previewModalHdr}>
            <Text style={s.previewModalTitle}>Event Preview</Text>
            <TouchableOpacity style={s.previewCloseBtn} onPress={()=>setShowPreview(false)}>
              <Ionicons name="close" size={20} color={COLORS.navy}/>
            </TouchableOpacity>
          </View>
          <Text style={s.previewModalSub}>This is how your event will appear to users</Text>
    ...
    <View style={pc.card}>
      ...
      <View style={pc.body}>
        <View style={pc.topRow}>
          ...
          {price==='Free'
            ?<View style={pc.freeBadge}><Text style={pc.freeBadgeTxt}>Free</Text></View>
...
const s = StyleSheet.create({
  ...
  scheduleGrid:{flexDirection:'row',flexWrap:'wrap',gap:8,marginBottom:8},
...
ENDOFFILE
echo "create-event.tsx written"

# Update events tab to show full card info
python3 << 'PYEOF'
content = open('app/(tabs)/events.tsx').read()

# Make sure user events show organizer and time
if 'organizer' not in content:
    content = content.replace(
        '<Text style={s.eventTitle} numberOfLines={2}>{event.title}</Text>',
        '<Text style={s.eventTitle} numberOfLines={2}>{event.title}</Text>\n                {!!(event as any).organizer && <Text style={s.eventOrganizer} numberOfLines={1}>by {(event as any).organizer}</Text>}'
    )
    content = content.replace(
        "  eventTitle:{fontSize:16,fontWeight:'700',color:COLORS.navy,lineHeight:22},",
        "  eventTitle:{fontSize:16,fontWeight:'700',color:COLORS.navy,lineHeight:22},\n  eventOrganizer:{fontSize:12,color:COLORS.gold,fontWeight:'600',marginTop:2},"
    )
